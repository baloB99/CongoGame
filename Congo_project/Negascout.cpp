...
int alphaBeta(string gameState, int depth, int alpha, int beta)
{
    ...
    {   
        // fstream myfile;
        // myfile.open("siba.txt", fstream::app);
...
int Negascout( string gameState, int depth, int alpha, int beta){
    vector<string> arr =tokenize(gameState, " ");
    ...
    for(int i=0; i < moves.size(); i++){
        ...
        string nextState = makemove(matrix_of_congo, moves[i]);
        int eval = -Negascout(nextState, depth - 1, -beta, -alpha);
        ...
        if( eval > beta){
            if( depth <= 2){
                ...
                eval = -Negascout(nextState, depth - 1, -beta, -eval);
            }
        }
        if( eval > alpha){
            alpha = eval;
...
void storeFens(vector<string> fens)
{
    for (int i=0; i < fens.size(); i++)
    {
        ...
        cout << alphaBeta(fens[i], 2, -1000000, 1000000) <<endl;
